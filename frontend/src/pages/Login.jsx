...
const Login = () => {
  ...
  return (
    <div className='flex flex-col gap-3 items-start justify-center mt-24 mb-[250px] mx-auto border border-gray-200 rounded-xl shadow-md px-8 py-8 w-[350px] sm:w-[380px] max-h-[600px]'>
      ...
      <form onSubmit={onSubmitHandler} className='flex flex-col items-center gap-3 ' >
        {
          state==="Login"?"":
          <label className='flex flex-col gap-1 text-sm  text-gray-600'>
          Full Name
          <input  className='w-full px-12 sm:px-16 py-[8px] rounded border border-gray-300 ' onChange={(e)=>setName(e.target.value)} value={name} type="text" />
        </label>
        }
        <label className='flex flex-col gap-1 text-sm  text-gray-600'>
          Email
          <input className='px-12 sm:px-16 py-[8px] rounded border border-gray-300 text-start' onChange={(e)=>setEmail(e.target.value)} value={email} type="email" />
        </label>

        <label className='flex flex-col gap-1 text-sm text-gray-600 mb-3'>
          Password
          <input className='px-12 sm:px-16 py-[8px] rounded border border-gray-300 ' onChange={(e)=>setPassword(e.target.value)} value={password}  type="password" />
        </label>
